...
from .models import User, UserConfirmation, PHOTO_DONE
from .models import VIA_EMAIL, VIA_PHONE, NEW, CODE_VERIFIED, DONE
...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def validate_email_phone_number(value):
        ...
        if value and User.objects.filter(email=value).exists():
            ...
            data = {
                "success": False,
                "message": "This phone number is already in the database."
            }
            raise ValidationError(data)
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate_username(self, username):
        if len(username) < 5 or len(username) > 30:
            raise ValidationError(
                {
                    "message": 'Username mst be between 5 and 30 characters long'
                }
            )
        if username.isdigit():
            raise ValidationError(
